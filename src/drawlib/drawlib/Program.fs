...
module lib3d = 
    ...
    type VSByteCode = { Data : byte[] }
    type PSByteCode = { Data : byte[] }

    let vs device file = 
        let bytecode = shaderByteCode file "VS" "vs_5_0"
        (new VertexShader(device, bytecode), {VSByteCode.Data = bytecode})
    
    let ps device file = 
        let bytecode = shaderByteCode file "PS" "ps_5_0"
        (new PixelShader(device, bytecode), {PSByteCode.Data = bytecode})

    let layout device {VSByteCode.Data = vsByteCode} (inputs:seq<VsInputElement>) = 
        let realInputs = 
            inputs 
            |> Seq.map (fun x -> 
